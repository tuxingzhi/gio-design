...
const PropertyPicker: React.FC<PropertyPickerProps> = (props: PropertyPickerProps) => {
  ...
  const debounceSetDetailVisible = debounce((visible: boolean) => {
  ...
  }, [disabledValues, originDataSource]);
  ...
  }, [scope, dataList, recentlyUsedInMemo, keyword]);
  ...
  function subGroupRender(groupData: Dictionary<PropertyItem[]>) {
    const dom = keys(groupData).map((gkey) => {
      const { groupName } = groupData[gkey][0];
      const listItems = getListItems(groupData[gkey]);
      return <List.ItemGroup title={groupName} subgroups={listItems} expandable />;
      // return (
  ...
  const renderItems = () => {
    if (propertyItems?.length === 0) {
      return <Page {...rest.emptyPrompt} />;
    }
    const recentlyNodes = recentlyPropertyItems?.length > 0 && (
      <List.ItemGroup title="最近使用" />
